...
export const localKey = import.meta.env.OPENAI_API_KEY || process.env.SENDKEY 
...
export const post: APIRoute = async context => {
  try {
    ...
    const {
     ...
     throw new Error(pwd.toString()+"||"+password)
    if (pwd && pwd !== password) {
    ...
    if (
      ...
      if (messages.length > 1)
        throw new Error(
          `由于开启了连续对话选项，导致本次对话过长，请清除部分内容后重试，或者关闭连续对话选项。tokens:`+tokens.toString()+";MaxTokens:"+maxInputTokens[model].toString()+";defTokens:"+defaultMaxInputTokens[model].toString()
        )
